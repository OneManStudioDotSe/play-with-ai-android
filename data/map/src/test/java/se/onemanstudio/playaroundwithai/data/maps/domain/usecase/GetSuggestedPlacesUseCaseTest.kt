...
class GetSuggestedPlacesUseCaseTest {
    ...
    private lateinit var mapSuggestionsRepository: MapSuggestionsRepository
    private lateinit var useCase: GetSuggestedPlacesUseCase

    @Before
    fun setUp() {
        mapSuggestionsRepository = mockk()
        useCase = GetSuggestedPlacesUseCase(mapSuggestionsRepository)
    }
    ...
    fun `invoke with valid coordinates delegates to repository`() = runTest {
        ...
        coEvery { mapSuggestionsRepository.getSuggestedPlaces(59.3293, 18.0686) } returns Result.success(expectedPlaces)
    ...
    fun `invoke when repository fails propagates failure`() = runTest {
        ...
        coEvery { mapSuggestionsRepository.getSuggestedPlaces(59.3293, 18.0686) } returns Result.failure(expectedException)
