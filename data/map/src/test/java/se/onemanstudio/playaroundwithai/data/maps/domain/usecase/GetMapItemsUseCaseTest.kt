...
class GetMapItemsUseCaseTest {
    ...
    private lateinit var mapPointsRepository: MapPointsRepository
    private lateinit var useCase: GetMapItemsUseCase

    @Before
    fun setUp() {
        mapPointsRepository = mockk()
        useCase = GetMapItemsUseCase(mapPointsRepository)
    }
    ...
    fun `invoke with valid params delegates to repository`() = runTest {
        ...
        coEvery { mapPointsRepository.getMapItems(2, 59.3293, 18.0686) } returns expectedItems
        ...
        coVerify(exactly = 1) { mapPointsRepository.getMapItems(2, 59.3293, 18.0686) }
    }
    ...
    fun `invoke when repository returns empty list returns empty list`() = runTest {
        coEvery { mapPointsRepository.getMapItems(any(), any(), any()) } returns emptyList()
