...
class GetSuggestedPlacesUseCaseTest {
    ...
    private lateinit var exploreSuggestionsRepository: ExploreSuggestionsRepository
    private lateinit var useCase: GetSuggestedPlacesUseCase

    @Before
    fun setUp() {
        exploreSuggestionsRepository = mockk()
        useCase = GetSuggestedPlacesUseCase(exploreSuggestionsRepository)
    }
    ...
    fun `invoke with valid coordinates delegates to repository`() = runTest {
        ...
        coEvery { exploreSuggestionsRepository.getSuggestedPlaces(59.3293, 18.0686) } returns Result.success(expectedPlaces)
    ...
    fun `invoke when repository fails propagates failure`() = runTest {
        ...
        coEvery { exploreSuggestionsRepository.getSuggestedPlaces(59.3293, 18.0686) } returns Result.failure(expectedException)
